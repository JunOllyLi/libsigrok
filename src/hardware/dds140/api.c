...
static const struct dds140_profile dev_profiles[] = {
	{
		0, 0, 0x8312, 0x8312, 0,
		"Sainsmart", "DDS140", "no_dds140.fw",
...
static struct sr_dev_inst *dds140_dev_new(const struct dds140_profile *prof)
{
...
static GSList *scan(struct sr_dev_driver *di, GSList *options)
{
	...
	const struct dds140_profile *prof;
	...
	for (i = 0; devlist[i]; i++) {
		...
		prof = NULL;
		for (j = 0; dev_profiles[j].fw_vid; j++) {
			if (des.idVendor == dev_profiles[j].fw_vid
				&& des.idProduct == dev_profiles[j].fw_pid) {
				/* Device matches the post-firmware profile. */
				prof = &dev_profiles[j];
				sr_dbg("Found a %s %s.", prof->vendor, prof->model);
				sdi = dds140_dev_new(prof);
...
static uint32_t data_amount(const struct sr_dev_inst *sdi)
{
	...
	return 0x2000;//data_left_2; // TODO
}
...
static int read_channel(const struct sr_dev_inst *sdi, uint32_t amount)
{
	...
	ret = dds140_get_channeldata(sdi, receive_transfer, amount);
...
static int handle_event(int fd, int revents, void *cb_data)
{
	...
	if (devc->dev_state == STOPPING) {
		...
		dds140_stop_data_collecting(sdi);
		/*
...
static int dev_acquisition_start(const struct sr_dev_inst *sdi)
{
	...
	dds140_start_data_collecting(sdi);
...
static struct sr_dev_driver dds140_driver_info = {
	.name = "DDS140",
	.longname = "SainSmart DDS140",
	.api_version = 1,
	.init = std_init,
	.cleanup = std_cleanup,
	.scan = scan,
...
SR_REGISTER_DEV_DRIVER(dds140_driver_info);
